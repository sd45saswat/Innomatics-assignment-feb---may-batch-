...
# ── DAY 4 — Task 5: Inventory Audit summary ───────────────────────
...
def get_inventory_audit():
    ...
    # Stock Value = sum of (price * 10) for each in-stock item
    total_val = sum(p['price'] * 10 for p in in_stock)
    
    # Find most expensive
    most_expensive_prod = max(products, key=lambda x: x['price'])
    ...
    return {
        ...
        "most_expensive": {
            "name": most_expensive_prod['name'],
            "price": most_expensive_prod['price']
        }
    }

# ── DAY 4 — Bonus: Category Discount ──────────────────────────────
...
def apply_category_discount(
    ...
    target_products = [p for p in products if p['category'].lower() == category.lower()]
    ...
    for p in target_products:
        ...
        updated_items.append({"name": p['name'], "new_price": p['price']})
